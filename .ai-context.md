...
- `package.json`: project metadata and dev scripts. Includes a `create-node-modules` script that copies `@airgap/beacon-sdk` into `src/node_modules` for playground/demo purposes.
...
- The project uses `@airgap/...` scoped packages widely. The most common scoped packages are `@airgap/beacon-sdk` and `@airgap/beacon-*` packages (dapp, core, types, ui, wallets, blockchains, transports, etc.).
...
- `package.json` (dependency: `@airgap/beacon-sdk` and `create-node-modules` script)
- `scripts/fix-beacon-sdk.js` (fixes paths for `@airgap/beacon-sdk` artifacts)
...
- Main SDK: `@airgap/beacon-sdk` -> `@tezos-x/octez.connect-sdk` (consumer imports like `import { DAppClient } from "@airgap/beacon-sdk"` should be updated)
...
- Script references that copy or patch `@airgap/beacon-sdk` artifacts -> update to `@tezos-x/octez.connect-sdk` and new artifact names.
...
2. Replace package import strings in docs and examples (MDX and TS files) from `@airgap/...` to `@tezos-x/octez.connect-...` and `@airgap/beacon-sdk` to `@tezos-x/octez.connect-sdk`.
