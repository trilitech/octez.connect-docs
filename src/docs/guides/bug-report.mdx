...
octez.connect includes a bug reporting feature, designed to gather and transmit user generated issue. This document outlines the structure and usage of the bug reporting system.

<DynamicImage src="/img/bug_report_alert.png" alt="octez.connect Pairing" />
...
```ts
{
    ...
    localStorage: string, // A stringified version of the user's octez.connect local storage data. octez.connect's local storage often contains user settings and other information that could be relevant in understanding the bug.
...
octez.connect creates a copy of the user's storage data at the moment of disconnection. This snapshot includes a copy of local and WalletConnect storage data at the time of disconnection.
