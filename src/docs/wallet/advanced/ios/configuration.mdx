...
octez.connect iOS SDK is not a stateless library and bases part of its functionality on preserved data. By default the SDK stores its data in [`UserDefaults`](https://developer.apple.com/documentation/foundation/userdefaults). However, it is possible to override the default storage and provide your own implementation. The `BeaconCore` package ships with 3 public protocols that can be satisfied and provided externally during the `Beacon.WalletClient` setup: `Storage`, `ExtendedStorage` (an extended version of `Storage`) and `SecureStorage`.
