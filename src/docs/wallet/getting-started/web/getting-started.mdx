...
First, you need to install the `octez.connect-sdk` package.

`npm install --save @tezos-x/octez.connect-sdk`

After that you need to import the octez.connect SDK in your code and initialize the WalletClient.

`octez.connect-sdk` will automatically establish a connection and listen to all octez.connect messages.
