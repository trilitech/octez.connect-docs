...
### Use octez.connect without taquito

When `octez.connect` is used without taquito, no connection to any RPC is required. So if your dApp offers only simple functionality (eg. only calling the `mint` entrypoint with one parameter), it is more performant to construct the contract call yourself and send it to the octez.connect Network direcly, without going through taquito first.
...
More information in the official [taquito RPC Caching Docs](https://taquito.io/docs/rpc-cache)
...
More information in the official [taquito Contracts Library Docs](https://taquito.io/docs/contracts-library)
...
More information in the official [taquito Local Packing Docs](https://taquito.io/docs/maps_bigmaps/#local-packing-for-big-maps)
