...
There are a few best practices dApp developers can follow to make their dApps more performant. We will go into a few of those here. Which ones are relevant to your dApp is always depending on the type of dApp. If you need help, feel free to reach out to us on [Discord](https://discord.gg/tezos) in the #beacon channel.

## Make sure octez.connect is up to date

The `octez.connect` SDK receives frequent updates with small bugfixes and performance improvements. We keep breaking changes to a minimum, so updating is usually as easy as increasing the version number.

## Reusing connected accounts

This one is a basic concept of octez.connect, but still very important.
...
In case your dApp is focussed around a specific time (eg. NFT drop or a countdown of some sorts), you should already provide a way for users to pair their wallet with the dApp. This will reduce the load on the octez.connect Network once the countdown hits 0.
...
Whenever doing a call in octez.js, it internally has to fetch data from the chain. Under normal circumstances, those requests are not a big deal. But if a high number of users are using your dApp at the same time, those requests can become a problem for the RPCs.

### Use octez.connect without octez.js

When `octez.connect` is used without octez.js, no connection to any RPC is required. So if your dApp offers only simple functionality (eg. only calling the `mint` entrypoint with one parameter), it is more performant to construct the contract call yourself and send it to the octez.connect Network direcly, without going through octez.js first.
...
### Use Caching with octez.js

octez.js offers a couple of optional features to reduce the number of network requests.
...
More information in the official [octez.js RPC Caching Docs](https://octez-js.tezos.com/docs/rpc-cache)
...
More information in the official [octez.js Contracts Library Docs](https://octez-js.tezos.com/docs/contracts-library)
...
More information in the official [octez.js Local Packing Docs](https://octez-js.tezos.com/docs/maps_bigmaps/#local-packing-for-big-maps)
...
## Check octez.connect Network Status

You can check the performance of the octez.connect Network here in our [Dashboard](https://status.tezos.com/). If you see spikes, it is possible that performance is affected. Usually, the network recovers once the load reduces.

:::info
When using Browser Extension wallets (eg. Temple or Spire), the status of the octez.connect Network does **NOT** affect the connection to the wallet. If something doesn't work in conjunction with Temple or Spire, it is most likely the RPC of the dApp or Wallet.
:::

## Reach out to the octez.connect Team

If you have any questions or doubts about the integration of octez.connect on your dApp, feel free to reach out to us on [Discord](https://discord.gg/tezos) in the #beacon channel. If are developing a dApp with a scheduled event, please contact us in advance so we have enough time to help you prepare for the release.
